...
\date{March 27, 2024}    % type date between braces
\maketitle
...
with each name not exceeding 130 characters in length.  For the special cases of one-variable files (\texttt{MR\_iwind}=5),
%for the NCEP/NCAR
...
files with different variable and dimension names, which may be incompatable.
...
Line 7 contains an integer flag specifing the output grid type
...
\begin{figure}[htbp]\begin{center}
 ...
 Example of foreward trajectory output using NCEP reanalysis (\texttt{MR\_iwindformat=25}) and
 6-point radiosonde data (left and right, respectively) using \texttt{RegridMet} with control files
...
\subsection{Windfile intercomparison}
\begin{figure}[htbp]\begin{center}
